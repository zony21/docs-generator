...
- Output file: sheets/Footnote.md
...
### 4.1 Reference terms and annotations
| Term | Description |
| --- | --- |
{{#each terms}}
| {{markdownCell term}} | {{markdownCell description}} |
{{else}}
|  |  |
{{/each}}

### 4.2 Abbreviations/codes
| Abbreviation/Code | Definition |
| --- | --- |
{{#each abbreviations}}
| {{markdownCell code}} | {{markdownCell definition}} |
{{else}}
|  |  |
{{/each}}

### 4.3 Supplemental notes
{{#each supplementalNotes}}
- {{this}}
{{else}}
No concrete entries.
{{/each}}
